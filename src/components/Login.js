...
const Login = () => {
  ...
  return (
    ...
    <div className="relative"> 
      <img src={BG_IMAGE} alt="background-img" className="w-full h-screen object-cover" />
      <div className="form-wrapper absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/75 rounded-md p-8 sm:w-96 md:w-1/3 lg:w-1/4"> 
        ...
        <form onSubmit={(e) => e.preventDefault()} className="">
          ...
          <input type="checkbox" id="remember-me" className="mr-2 accent-gray-400" />
          <label className="text-gray-300 text-sm">Remember me</label>
        </div>

          </div>
        </form>
        <p className="text-gray-300 text-sm mt-4 cursor-pointer" onClick={toggleSignInForm}>{isSignInForm ? "New to Netflix? Sign up now" : "Already a User? Sign In Now"}
    </p>
      </div>
    </div>
  </div>
